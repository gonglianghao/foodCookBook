...
router.post('/login',async (ctx)=>{
   ...
   try {            
     const result  = await userLogin(phone ,password) //调用后应接.then，但可以选择await
...
router.post('/register',async(ctx)=>{
  //  拿到前端传过来的 username,password,nickname
   // 到数据库中校验 username 是否存在，若不存在往数据库植入新的数据
     const {username,password,nickname,phone} = ctx.request.body  //请求体
    // 判断传递消息是否为空
     ...
     try {
      const findRes = await userFind(phone)
      ...
      // 植入操作
      const registerRes = await userRegister([username,password,nickname,phone])
